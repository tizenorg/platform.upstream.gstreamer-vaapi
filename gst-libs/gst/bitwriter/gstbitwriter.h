...
GstBitWriter *  gst_bit_writer_new               (guint32 reserved_bits) G_GNUC_MALLOC;
GstBitWriter *  gst_bit_writer_new_fill          (guint8 *data, guint bits) G_GNUC_MALLOC;
void            gst_bit_writer_free              (GstBitWriter *writer, gboolean free_data);

void            gst_bit_writer_init              (GstBitWriter *bitwriter, guint32 reserved_bits);
void            gst_bit_writer_init_fill         (GstBitWriter *bitwriter, guint8 *data, guint bits);
void            gst_bit_writer_clear             (GstBitWriter *bitwriter, gboolean free_data);

guint           gst_bit_writer_get_size          (GstBitWriter *bitwriter);
guint8 *        gst_bit_writer_get_data          (GstBitWriter *bitwriter);
gboolean        gst_bit_writer_set_pos           (GstBitWriter *bitwriter, guint pos);
guint           gst_bit_writer_get_space         (GstBitWriter *bitwriter);
...
_gst_bit_writer_check_space(GstBitWriter *bitwriter, guint32 bits)
{
    ...
    g_assert(new_bit_size && ((new_bit_size&__GST_BITS_WRITER_ALIGNMENT_MASK) == 0));
...
)
{
    if ((bitwriter->bit_size & 0x07) == 0) {
        memcpy(&bitwriter->data[bitwriter->bit_size >> 3],
               data, nbytes);
...
static inline void
gst_bit_writer_align_bytes_unchecked(GstBitWriter *bitwriter, guint8 trailing_bit)
{
...
static inline gboolean
_gst_bit_writer_put_bytes_inline(
    GstBitWriter *bitwriter,
    const guint8 *data,
    guint nbytes
)
...
static inline gboolean
_gst_bit_writer_align_bytes_inline(GstBitWriter *bitwriter, guint8 trailing_bit)
{
    g_return_val_if_fail(bitwriter != NULL, FALSE);
