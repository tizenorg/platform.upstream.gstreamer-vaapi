...
/**
 * gst_vaapi_image_ensure_mapped_buffer:
 * @image: a #GstVaapiImage
 *
 * initialize mapped image buffer in case of optimizition on
 * nothing did between map and unmap operations.
 *
 * Return value: %TRUE on success
 */

gboolean
gst_vaapi_image_ensure_mapped_buffer(GstVaapiImage *image)
{
    GstVaapiImagePrivate * const priv = image->priv;
    guchar *user_ptr;
    guchar tmp;
    ...
    g_return_val_if_fail(GST_VAAPI_IS_IMAGE(image), FALSE);
    g_return_val_if_fail(priv->is_constructed, FALSE);

    if (!_gst_vaapi_image_is_mapped(image) || !priv->image_data)
        return FALSE;

#undef GST_VAAPI_IMAGE_PAGE_SIZE
#define GST_VAAPI_IMAGE_PAGE_SIZE 4096

    /* read each page make sure virtual buffer mapped*/
    user_ptr = priv->image_data;
    for (index = 0;
         index < priv->image.data_size;
         index += GST_VAAPI_IMAGE_PAGE_SIZE) {
        tmp = *(user_ptr + index);
        /*prevent from compiling optimization*/
        if (tmp == 0)
            *(user_ptr + index) = 0;
    }
