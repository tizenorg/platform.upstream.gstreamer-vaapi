    ...
    GstCaps      *caps)
{
    ...
    sink->video_pool = gst_vaapi_surface_pool_new(sink->display, caps);
    if (!sink->video_pool) {
        return FALSE;
    }
    ...
    GstBuffer   **buf)
{
    ...
    if (!gst_vaapisink_ensure_video_pool(sink, caps))
        goto error;

    video_buffer = gst_vaapi_video_buffer_new_from_pool(sink->video_pool);
...
error:
    GST_ERROR("vaapisink allocate buffer failed.");
