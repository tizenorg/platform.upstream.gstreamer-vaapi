...
#if USE_WAYLAND
static gboolean
gst_vaapisink_ensure_window_wlid(GstVaapiSink *sink, guintptr window_id);
#endif
...
gst_vaapisink_video_overlay_set_window_handle(GstVideoOverlay *overlay, guintptr window)
{
    ...
    case GST_VAAPI_DISPLAY_TYPE_WAYLAND:
        gst_vaapisink_ensure_window_wlid(sink, window);
        break;
#endif
    default:
        break;
...
gst_vaapisink_ensure_window_wlid(GstVaapiSink *sink, guintptr window_id)
{
    ...
    switch (sink->display_type) {
	case GST_VAAPI_DISPLAY_TYPE_WAYLAND:
        sink->window = gst_vaapi_window_wayland_new_with_id(sink->display, wlid);
        break;
    default:
        GST_ERROR("unsupported display type %d", sink->display_type);
        return FALSE;
...
gst_vaapisink_class_init(GstVaapiSinkClass *klass)
{
     ...
     *
     * todo, use 'display' for this one, and use 'display-type' for the display-type string
     */
#if USE_WAYLAND
    g_object_class_install_property
        (object_class,
