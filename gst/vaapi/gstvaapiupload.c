...
)
{
    ...
    /* PVR need directly copy image to surface */
    ...
    if (!upload->need_manual_upload) {
        success = gst_vaapi_image_update_from_buffer(image, inbuf, NULL);
...
error_put_image:
    {
        ...
        if (!upload->direct_rendering && image)
            g_object_unref(image);
        return GST_FLOW_OK;
    }
}
...
)
{
    ...
    gst_vaapi_video_pool_set_capacity(upload->surfaces, 6);
    gst_vaapi_video_pool_reserve(upload->surfaces, 4);
